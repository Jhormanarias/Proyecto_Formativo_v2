...
namespace ProyectoFormativo
{
    public partial class FrmControlAdmin : Form
    {
        ...
        private void btnBuscarP_Click(object sender, EventArgs e)
        {
            if (txtBuscar_VerP.Text == "N Documento: ")
            {
                MessageBox.Show("Inserte algo", "No hay nada que buscar", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                if (txtBuscar_VerP.Text != "")
                {
                    DataTable tablaPropietario = ClaseControlAdmin.Func_TraerPropietario(Convert.ToInt64(txtBuscar_VerP.Text));
                    dgvPropietario.DataSource = tablaPropietario;
                    if (tablaPropietario.Rows.Count == 0)
                    {
                        MessageBox.Show("Esta persona no Existe", "Advertencia!!!", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
        ...
        private void dgvPropietario_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (n != -1)
            {
                long doc = Convert.ToInt64(dgvPropietario.Rows[n].Cells[4].Value);
                DataTable tablaPropietario = ClaseControlAdmin.Func_TraerPropietario(doc);
                id_prop = Convert.ToInt64(tablaPropietario.Rows[0][0]);
                ...
                //------------colors buttons-----------
                btnModificarP.BackColor = System.Drawing.SystemColors.MenuHighlight;
                btnModificarP.ForeColor = System.Drawing.SystemColors.HighlightText;
                btnEliminarP.BackColor = System.Drawing.SystemColors.MenuHighlight;
                btnEliminarP.ForeColor = System.Drawing.SystemColors.HighlightText;
        ...
        {
            controladd = 1;
            string name = dgvPropietario.Rows[n].Cells[1].Value.ToString();
            DialogResult rpt = new DialogResult();
            rpt = MessageBox.Show("Esta seguro que quiere modificar el propietario " + name + "?", "Advertencia", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
            if (rpt == DialogResult.OK)
            {
                txtNombreP.Text = dgvPropietario.CurrentRow.Cells[1].Value.ToString();
                txtApellidoP.Text = dgvPropietario.CurrentRow.Cells[2].Value.ToString();
                cbTipoDocumentoP.Text = dgvPropietario.CurrentRow.Cells[3].Value.ToString();
                txtNDocumentoP.Text = dgvPropietario.CurrentRow.Cells[4].Value.ToString();
                txtCorreoP.Text = dgvPropietario.CurrentRow.Cells[5].Value.ToString();
                txtTelefonoP.Text = dgvPropietario.CurrentRow.Cells[6].Value.ToString();
        ...
        private void btnEliminarP_Click(object sender, EventArgs e)
        {
            string name = dgvPropietario.Rows[n].Cells[1].Value.ToString();
            DialogResult rpt = new DialogResult();
            rpt = MessageBox.Show("Esta seguro de que quieres eliminar el usuario "+ name +"?", "Advertencia", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
            if (rpt == DialogResult.OK)
            {
                if (ClaseControlAdmin.Func_DeletePropietario(id_prop))
                {
                    MessageBox.Show("Propietario eliminado", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
        ...
        private void btnNuevoP_Click(object sender, EventArgs e)
        {
            ...
            {
                dgvPropietario.Rows.RemoveAt(dgvPropietario.CurrentRow.Index);
            }
